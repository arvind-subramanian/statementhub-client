contract UserProfile {
  ...
  mapping (uint256 => uint256) private emailPersonMap;
  ...
  function addPerson(uint256 nameHash, uint256 emailHash,bool emailverified, uint32 personRole, uint32 reputation) external payable {
    User memory newUser = User(nameHash, emailverified, emailHash, personRole, reputation);
    uint256 personId = userList.push(newUser) -1;
